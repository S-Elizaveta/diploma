...
class EventPage extends StatefulWidget {
  ...
  EventPage({Key key, this.notes, this.noteList}) : super(key: key);

  @override
  _EventPageState createState() => _EventPageState(notes, noteList);
}
...
class _EventPageState extends State<EventPage> {
  ...
  List<EventPageMessages> _eventList;

  _EventPageState(this._notes, this._noteList);

  @override
  void initState() {
    super.initState();
    BlocProvider.of<CubitEventPage>(context).init(_notes);
        ...
        if (state.currentEventsList[index].imagePath == null)
          IconButton(
            ...
            onPressed: () {
              ...
              _textInputController.text = state.currentEventsList[index].text;
              _textInputController.selection = TextSelection(
                baseOffset: _textInputController.text.length,
                extentOffset: _textInputController.text.length,
              );
            },
          ),
        if (state.currentEventsList[index].imagePath == null)
  ...
  RadioListTile _radioListTile(StatesEventPage state, int index) {
    return RadioListTile(
      title: Text(_noteList[index].notesTitle),
      value: index,
  ...
  Widget _showEventList(StatesEventPage state,  EventPageMessages event, int index) {
    return Padding(
      padding: EdgeInsets.only(
        left: 10.0,
        right: 10.0,
        ...
        IconButton(
          ...
          onPressed: () {
            if (state.isSendPhotoButton) {
              ...
              state.isEditing
                  ? BlocProvider.of<CubitEventPage>(context)
                  .editText(state.selectedItemIndex, _textInputController.text)
                  : BlocProvider.of<CubitEventPage>(context)
                  .addEvent(_textInputController.text);
  ...
  void updateNoteSubtitle() {
    ...
    if (state.currentEventsList.isNotEmpty) {
      if (state.currentEventsList[0].imagePath == null) {
        state.note.notesSubtitle =
        '${state.currentEventsList[0].text}  ${state.currentEventsList[0].time}';
      } else {
        state.note.notesSubtitle = 'Image';
      }
    } else {
      state.note.notesSubtitle = 'No events. Click to create one';
    }
    BlocProvider.of<CubitEventPage>(context).updateNote();
  }
